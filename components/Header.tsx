      ...
      <div className="bg-[#c1121f] text-white">
        <div className="mx-auto max-w-6xl px-4 py-2 text-xs sm:text-sm">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            <span className="opacity-80">•</span>
            <a
              className="break-all underline-offset-2 hover:underline sm:break-normal"
      ...
      <div className="mx-auto max-w-6xl px-4 py-4">
        <div className="flex min-w-0 items-center justify-between gap-3">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            ...
            <Link
              href="/contact"
              className="rounded-xl bg-[#c1121f] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90"
            >
              Request a Quote
            </Link>
          </div>
        ...
        <nav className="mt-4 hidden gap-6 text-sm font-semibold sm:flex">
          {navLinks.map((l) => (
            <Link key={l.href} href={l.href} className="text-black/70 transition hover:text-black">
              {l.label}
            </Link>
          ))}
        </nav>
              ...
              <div className="flex flex-col gap-3">
                {navLinks.map((l) => (
                  <Link
                    key={l.href}
                    href={l.href}
                    className="rounded-xl px-3 py-2 font-semibold text-black/80 transition hover:bg-black/5"
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  ...
                  <Link
                    href="/contact"
                    className="rounded-xl bg-[#c1121f] px-4 py-2 text-center text-sm font-semibold text-white transition hover:opacity-90"
                    onClick={() => setOpen(false)}
                  >
                    Quote
                  </Link>
